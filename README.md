...
#### 1. MySQLコンテナへの接続
Laravel Sailで起動しているMySQLコンテナ（rootユーザー）に接続します。パスワードには `password` を入力してください。

```bash
./vendor/bin/sail mysql -u root -p
```

#### 2. テスト用データベースを作成し、権限を付与します。
```sql
CREATE DATABASE IF NOT EXISTS bookshelf_test CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
GRANT ALL ON bookshelf_test.* TO 'sail'@'%';
FLUSH PRIVILEGES;
EXIT;
```

#### 3. テストを実行します。
テストは `RefreshDatabase` トレイトにより実行のたびにマイグレーションが自動適用されます。
